...
export async function POST(req){
  try{
    ...
    if (!toStr(reversed_by))
      return err(400,"Missing Field","'reversed_by' (user_id) is required.");
    ...
    if (capture.reversed === true)
      return err(409, "Already Reversed", "This stock capture has already been reversed.");
    ...
    if (!authorisedUsers.includes(toStr(reversed_by))) {
      return err(
        403,
        "Permission Denied",
        `You are not authorised to reverse stock adjustments for location '${locData.title || location_id}'.`
      );
    }
    ...
    /* ---------- 4. Mark original capture as reversed ---------- */
    ...
    const reversalRef = await addDoc(collection(db, "stock_reversals"), reversalRecord);

    /* ---------- 6. Respond ---------- */
    return ok({
      message: `Stock reversal completed for ${reversedProducts.length} product(s) at location '${locData.title || location_id}'.`,
      data: {
