...
/** Scan categories and see if any data.category.slug equals target (case-insensitive) */
async function slugTaken(targetSlugNorm, excludeSlugNorm = "") {
  const snap = await getDocs(collection(db, "categories"));
  for (const d of snap.docs) {
    const data = d.data() || {};
    const catSlug = norm(data?.category?.slug);
    if (!catSlug) continue;
    if (excludeSlugNorm && catSlug === excludeSlugNorm) continue; // allow current slug during edits
    if (catSlug === targetSlugNorm) return true;
  }
  return false;
}
